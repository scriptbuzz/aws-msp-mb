...
> Status: **deployed & operational** — provisioned in the sandbox account and the
> first release ran the pipeline end-to-end to a live prod site (see §12 and
> [STATUS.md](STATUS.md)).
...
- **Repo:** `github.com/scriptbuzz/aws-msp-mb`
...
Local working copy: a local clone of the repo.
...
## 11. Manual / apply-time tasks

The human-required steps and **why each can't be automated** are catalogued in
[MANUAL_OPERATIONS.md](MANUAL_OPERATIONS.md). In brief:
...
1. Authorize the GitHub connection OAuth handshake in the AWS console (comes up `PENDING`),
   and grant the *AWS Connector for GitHub* app access to the repo (enables push auto-trigger).
2. Approve the production gate; attach the deny-direct-prod-deploy policy to SSO permission sets.
3. Provide AWS access via the project `.env` — **`AWS_PROFILE` (SSO), not long-lived keys**
   (see `.env.example` + AWS_ACCOUNT_SETUP.md).

> The Terraform-state buckets (previously a manual chicken-and-egg) are now automated:
> `scripts/rollout.sh apply bootstrap`. See [MANUAL_OPERATIONS.md](MANUAL_OPERATIONS.md) §B.

---

---

## 12. Rollout outcome (2026-07-03)

**Deployed to the sandbox account and the first release ran end-to-end.** Provisioned
in stages via [`scripts/rollout.sh`](../scripts/rollout.sh) (see [ROLLOUT.md](ROLLOUT.md)):
bootstrap buckets → shared (VPC/ECR/IAM/cluster/pipeline) → dev/test/stage → prod
(ALB + blue/green + CodeDeploy). The first pipeline execution went green across every
stage — Source → Build → Deploy-dev → Deploy-test → Deploy-stage → change-record →
**manual approval (human, "Looks good.")** → **prod CodeDeploy blue/green** — and the prod
site returned HTTP 200 serving the build-stamped version. This exercises all four
mandatory components plus the GAP items (staged promotion, approval gate, ITSM change
record, blue/green auto-rollback rig).

*Bugs found & fixed during rollout:* invalid `security_scan:` CodeBuild phase (moved to
a labeled block in `build`); immutable-tag re-run collision (releases bump `app/VERSION`);
Build artifact didn't ship the deploy/change-record buildspecs; CodePipeline role lacked
`codedeploy:GetDeploymentConfig` on the AWS-managed deployment config. *External blocker:*
a new-account CodeBuild hold (0 concurrent builds account-wide) — lifted by AWS Support.

*Remaining:* capture audit evidence into `docs/evidence/release-management/`; demo the
automated blue/green rollback via a deliberate health-check failure; attach the
deny-direct-prod-deploy policy to SSO permission sets; confirm push-based auto-triggering.

---

*Last updated: 2026-07-03 15:13. Implementation notes:* (1) A **`shared` environment root**
was added for the once-per-account layer (network, ECR, IAM, cluster, pipeline); env roots
read it via `terraform_remote_state`. (2) CodeBuild phase names are fixed, so the §8a
security-scan extension point is a **labeled block inside the `build` phase** of
`buildspec.yml` — same plug-in contract. (3) Network decision recorded: no NAT / no private
subnets; tasks in public subnets with restricted SGs.
